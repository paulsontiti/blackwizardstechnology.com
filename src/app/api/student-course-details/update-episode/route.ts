...
import {
  StudentCourseDetailsEpisodeType,
  StudentCourseDetailsType,
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (isBodyInValid(body)) {
      ...
      const courseDetails = await StudentCourseDetailsdeModel.findOne({
        studentId,
      });
      
      if (courseDetails) {
        //get episode
        const episode = courseDetails.coursesTaken
        .find(
          (course: StudentCourseDetailsType) =>
            course.courseId.toString() === courseId
        ).episodes.find(
            (ep: StudentCourseDetailsEpisodeType) =>
      ...
      .find(
        (course: StudentCourseDetailsType) =>
          ...
          const episode = {episodeNumber,score:value}
          courseDetails.coursesTaken
      .find(
        (course: StudentCourseDetailsType) =>
          course.courseId.toString() === courseId
